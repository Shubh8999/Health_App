...
class Display:
    ...
    def display_overview(self,recommendations):
        if recommendations!=None:
            ...
            selected_recipe_name = None  # Define the variable with a default value
            with col2:
                if recommendations is not None and all(isinstance(recipe, dict) and 'Name' in recipe for recipe in recommendations):
            ...
            for recipe in recommendations:
                if isinstance(recipe, dict) and 'Name' in recipe and recipe['Name'] == selected_recipe_name:
            ...
            if selected_recipe_name is not None:
                ...
                st.write("Please select a recipe to display the nutrition values.")
...
with st.form("recommendation_form"):
    ...
    Calories = st.slider('Calories', 0, 2000, 0)
    FatContent = st.slider('FatContent', 0, 100, 0)
    SaturatedFatContent = st.slider('SaturatedFatContent', 0, 13, 0)
    CholesterolContent = st.slider('CholesterolContent', 0, 300, 0)
    SodiumContent = st.slider('SodiumContent', 0, 2300, 0)
    CarbohydrateContent = st.slider('CarbohydrateContent', 0, 325, 0)
    FiberContent = st.slider('FiberContent', 0, 50, 0)
    SugarContent = st.slider('SugarContent', 0, 40, 0)
    ProteinContent = st.slider('ProteinContent', 0, 40, 0)
